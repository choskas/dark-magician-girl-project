...
import { AllCardsWrapper, DeckWrapper, MyDeckWrapper, SearchAndMyDeckWrapper, SearchBothInputWrapper, SearchDeckWrapper, SearchInputWrapper } from "../../styles/deckPrice/deckPrice";
        ...
        if (value.length > 3) {
            ...
            card = allCards.find((el) => {
                if (el.card_sets !== undefined) {
                    return el.card_sets.find((item) => item.set_code === value.toUpperCase())
                }
            });
            
            if (card !== undefined) {
                const cardInfo = card.card_sets.find((el: any) => el.set_code === value.toUpperCase());
                setFoundCards([{
                    id: card.id,
                    name: card.name,
                    level: card.level,
            ...
            <SearchAndMyDeckWrapper>
                <AllCardsWrapper>
                    {foundCards && foundCards.map((item: any) => (
                        <Card isEditable={false} foundCards={foundCards} name={item.name} image={item.card_images[0].image_url_small} />
                    ))}
                </AllCardsWrapper>
                <CardFrameVertical />
                <MyDeckWrapper ref={drop} >
                    My Deck 
                    <DeckWrapper>
                    {myDeck && myDeck.map((item:any, index) => (
                        <Card
                            index={index}
